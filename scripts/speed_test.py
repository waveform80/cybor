...
Codec = namedtuple('Codec', ('cbor', 'cbor2', 'cboar', 'cybor'))
...
def main():
    ...
    for name, value in TEST_VALUES.items():
        ...
        results[name] = Codec(**{
            ...
            for mod in (cbor, cbor2, cboar, cybor)
        })
    ...
    rows = [head] + [
        (
            ...
            color_time(result.cybor.decoding.time, result.cbor2.decoding.time),
    ...
    print(''.join((
        ...
        ' ' * col_widths[0],
        ' +-',
        '-' * (sum(col_widths[1:5]) + 9),
        '-+-',
        '-' * (sum(col_widths[5:9]) + 9),
        '-+',
    )))
    print(''.join((
        '  ',
        ' ' * col_widths[0],
        ' | ',
        '{value:^{width}}'.format(value='Encoding', width=sum(col_widths[1:5]) + 9),
        ' | ',
        '{value:^{width}}'.format(value='Decoding', width=sum(col_widths[5:9]) + 9),
        ' |',
    )))
    print(sep)
    print(''.join((
